...
class BEIRHybridRetriever:
    ...
    def retrieve(self, queries, top_k=100):
        ...
        print(f"Retrieving with hybrid pipeline (cross_encoder={self.pipeline.cross_encoder is not None}, top_k={top_k})")
        results = {}
        
        for query_id, query_text in tqdm(queries.items()):
            # Use the hybrid pipeline
...
def save_results_to_files(dataset_name, results_summary, timestamp):
    ...
    with open(csv_filename, 'w', newline='') as f:
        ...
        # Write header
        header = ["Configuration", "nDCG@1", "nDCG@3", "nDCG@5", "nDCG@10", "nDCG@100",
                 "Recall@1", "Recall@3", "Recall@5", "Recall@10", "Recall@100",
                 "MAP@1", "MAP@3", "MAP@5", "MAP@10", "MAP@100",
                 "Precision@1", "Precision@3", "Precision@5", "Precision@10", "Precision@100"]
...
if __name__ == "__main__":
    ...
    # Different configurations to test hybrid retrieval
    CONFIGURATIONS = {
        "hybrid_baseline": {
            "cross_encoder": False,
            "bandit": False,
        ...
        "hybrid_dense_only": {
            "cross_encoder": False,
            "bandit": False,
            "top_k": 25  # Will effectively be dense-only with smaller retrieval
        },
